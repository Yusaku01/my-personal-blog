...
3. 2のフローの上で、プリフェッチ可能であれば、以下 3 つに分岐する（以下1 > 2 > 3の順にフォールバックする）
    1. Speculation Rules API に対応している場合は、`<script type="speculationrules">` を挿入する。
    2. `<link rel="prefetch">` を挿入する。
    3. `fetch()` を実行する。

ここで注目すべきは、AstroはブラウザのAPIを拡張する形でPrefetchを実現している点です。Speculation Rules APIなどを活用することで、ブラウザの最適化や制御の恩恵を受けることができます。
...
個別リンクごとにPrefetchを有効化したい場合は、対象リンクに `data-astro-prefetch` 属性を付与します。
...
前述したように、AstroのPrefetchは、configファイルで`prefetch`を設定して機能させますが[`clientPrerender`](https://docs.astro.build/en/reference/experimental-flags/client-prerender/)をtrueにすることで、Speculation Rules APIを活用したPrefetchも有効化されます。
